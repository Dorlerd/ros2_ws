...
class Subscriber(Node):
    ...
    def acceleration_init(self,msg):
        ...
        if self.k:
            self.ang_x = math.asin(self.ac_y/self.ac_z)
            self.ang_y = math.asin(self.ac_x/self.ac_z)
            self.k = False
    ...
    def velocity(self):
        ...
        accel = Vector3()
        accel.x = (self.ac_x - 9.81*math.sin(self.ang_y))*math.cos(self.ang_y)
        accel.y = (self.ac_y - 9.81*math.sin(self.ang_x))*math.cos(self.ang_x)
        accel.z = (self.ac_z - 9.81 * (math.cos(self.ang_x)* math.cos(self.ang_y)))
